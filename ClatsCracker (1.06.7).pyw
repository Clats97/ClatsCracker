...
_HASH_FUNCS = {
    ...
    'sm3': lambda b: hashlib.new('sm3', b).hexdigest(),'md5_sha1': lambda b: hashlib.new('md5-sha1', b).hexdigest(),'mdc2': lambda b: hashlib.new('mdc2', b).hexdigest(),'whirlpool': lambda b: hashlib.new('whirlpool', b).hexdigest(),
...
class HashCrackerGUI:
    def __init__(self, master: tk.Tk):
        self.master = master
        master.title("ClatsCracker Hash Cracking Tool GUI v1.07")
...
╚██████╗███████╗██║  ██║   ██║   ███████║╚██████╗██║  ██║██║  ██║╚██████╗██║  ██╗███████╗██║  ██║
 ╚═════╝╚══════╝╚═╝  ╚═╝   ╚═╝   ╚══════╝ ╚═════╝╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝
      C L A T S C R A C K E R      H A S H   C R A C K I N G    T O O L    (Version 1.07)
"""
        ...
        self.thr_var = tk.StringVar(value='8')
    ...
    def _add_dict(self):
        [self.dict_list.insert(tk.END, f) for f in filedialog.askopenfilenames(title="Select Dictionaries", filetypes=[("Text", "*.txt"), ("All", "*.*")]) if f not in self.dict_list.get(0, tk.END)]
    def _rem_dict(self):
        [self.dict_list.delete(i) for i in reversed(self.dict_list.curselection())]
    ...
    def _start(self):
        ...
        try:
            ...
            pass
        _POOL = ThreadPoolExecutor(max_workers=self.state.threads_count)
