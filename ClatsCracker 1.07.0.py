...
def _index_to_password(idx: int, chars: str, length: int) -> str:
    base = len(chars)
    buf = [''] * length
...
def _bf_worker(start: int, end: int, chars: str, length: int, d: str, a: str, report_step: int) -> Tuple[int, Optional[str]]:
    checker = compile_checker(d, a)
    processed = 0
    for idx in range(start, end):
        processed += 1
        if checker(_index_to_password(idx, chars, length)):
            return processed, _index_to_password(idx, chars, length)
...
def brute_force_crack(d: str, a: str, chars: str, length: int, state: CrackerState, cb, done):
    ...
    state.total_passwords = total
    write_log("Bruteforce started")
    task_size = 200_000
    report_step = 50_000
    ranges = [(i, min(i + task_size, total)) for i in range(0, total, task_size)]
    futs = { _POOL.submit(_bf_worker, s, e, chars, length, d, a, report_step) for s, e in ranges }
    while futs and not state.abort_requested:
        done_set, futs = wait(futs, return_when=FIRST_COMPLETED)
        for fut in done_set:
            proc, found = fut.result()
            with state.progress_lock:
                state.passwords_tried += proc
                pct = state.passwords_tried / total * 100
            cb(pct, state)
            if found:
