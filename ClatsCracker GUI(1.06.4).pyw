...
import hashlib, os, time, bcrypt, itertools, string, threading, zlib, logging
import concurrent.futures as cf
import psutil, tkinter as tk, multiprocessing as mp          # ← added mp
...
LOG_FILE = "Hash_Cracking_Results_ClatsCracker.txt"
...
_HASH_FUNCS = {
    ...
    'md4':   lambda b: hashlib.new('md4', b).hexdigest(),
    'md5':   lambda b: hashlib.md5(b).hexdigest(),
    'ripemd160': lambda b: hashlib.new('ripemd160', b).hexdigest(),
...
def guess_hash_algorithm(h: str) -> Optional[List[str]]:
    ...
    if l == 32 and all(c in '0123456789abcdef' for c in hl):
        return ['md5', 'ntlm']
    c = [k for k, v in hash_lengths.items() if l == v]
    if l == 128 and all(c in '0123456789abcdef' for c in hl):
        c.append('scrypt')
    return list(dict.fromkeys(c)) or None
...
def validate_hash_length(a: str, h: str) -> bool:
    if a in {'bcrypt', 'argon2id', 'scrypt', 'phpass_md5', 'ntlm'}:
        return True
    e = hash_lengths.get(a)
    return False if e and len(h) != e else True

def throttle_cpu():
    if psutil.cpu_percent(None) > CPU_USAGE_THRESHOLD:
        time.sleep(0.2)

def check_password_cpu(p: str, d: str, a: str) -> Tuple[bool, Optional[str]]:
    if a == 'bcrypt':
        try:
            if bcrypt.checkpw(p.encode(), d.encode()):
                return True, p
        except Exception:
            return False, None
        return False, None
...
def dictionary_crack_worker(path: str, d: str, a: str, state: CrackerState, step: int, cb) -> None:
...
def concurrent_hash_cracker(dicts: List[str], d: str, a: str, state: CrackerState, cb, done) -> None:
    ...
    if not state.total_passwords:
        ...
        return
    with ThreadPoolExecutor(max_workers=min(state.threads_count, len(dicts))) as tpe:
        futs = [tpe.submit(dictionary_crack_worker, p, d, a, state, 5000000, cb) for p in dicts if os.path.exists(p)]
        while True:
...
def brute_force_crack(d: str, a: str, chars: str, length: int, state: CrackerState, cb, done) -> None:
    state.found_password = None
    state.passwords_tried = 0
    state.abort_requested = False
    state.cracking_complete = False
    ...
    for chunk in _generate_passwords(chars, length):
        ...
        futs = [_POOL.submit(_process_chunk, s, d, a, 5000000) for s in subs]
...
def detect_file_encoding(path: str, sample: int = 10000) -> str:
    ...
    try:
        with open(path, 'rb') as f:
            enc = chardet.detect(f.read(sample)).get('encoding') or 'utf-8'
            return enc
    except Exception:
        return 'utf-8'
...
def lines_in_chunks(path: str, chunk: int = 100000):
    try:
        enc = detect_file_encoding(path)
        with open(path, 'r', encoding=enc, errors='replace') as f:
            batch = []
...
class HashCrackerGUI:
    def __init__(self, master: tk.Tk):
        self.master = master
        master.title("ClatsCracker Hash Tool GUI v1.06.4")
...
╚██████╗███████╗██║  ██║   ██║   ███████║╚██████╗██║  ██║██║  ██║╚██████╗██║  ██╗███████╗██║  ██║
 ╚═════╝╚══════╝╚═╝  ╚═╝   ╚═╝   ╚══════╝ ╚═════╝╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝
           C L A T S C R A C K E R     H A S H     T O O L   (Version 1.06.4)
          Multi-algorithm hash cracking tool. Ethical use only.
         🛡️By Josh Clatney - Ethical Pentesting Enthusiast  🛡️
        """
        self.branding_label = tk.Label(self.main_frame, text=branding_text, font=("Courier", 10, "bold"), justify="center", fg="blue", bg="lightgrey", anchor="center")
        ...
        self.algo_box = ttk.Combobox(
            ...
            values=['auto'] + sorted(hash_lengths) + ['bcrypt', 'scrypt', 'argon2id', 'phpass_md5', 'ntlm', 'ntplm', 'shake128', 'shake256', 'shakeke128']
        )
        ...
        self.thr_var = tk.StringVar(value='4')
        self.thr_box = ttk.Combobox(self.main_frame, textvariable=self.thr_var, state="readonly", values=['1 (Low)', '4 (Medium)', '8 (High)','Custom'])
    ...
    def _add_dict(self):
        for f in filedialog.askopenfilenames(title="Select Dictionaries", filetypes=[("Text", "*.txt"), ("All", "*.*")]):
    ...
    def _metrics(self):
        if self.state.start_time:
            e = max(1e-3, time.time() - self.state.start_time)
            aps = self.state.passwords_tried / e
        else:
            aps = 0.0
    ...
    def _progress_ui(self, pct: float, state: CrackerState):
        ...
        if state.start_time and state.passwords_tried:
            ...
            aps = state.passwords_tried / e
            rem = max(0, state.total_passwords - state.passwords_tried)
            eta = rem / aps if aps else float('inf')
    ...
    def _start(self):
        ...
        if self.method_var.get() == 'dictionary':
            ...
            if not dicts:
                ...
                return
            threading.Thread(target=concurrent_hash_cracker, args=(list(dicts), d, a, self.state, self._progress, self._done), daemon=True).start()
        else:
