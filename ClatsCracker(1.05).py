...
def phpass_verify(password: str, phpass_hash: str) -> bool:
    """
    Verify a password against a phpass (WordPress-like) MD5-based hash.

    Reference: https://www.openwall.com/phpass/
    """
    # The itoa64 alphabet as per phpass specification
    itoa64 = "./0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"

    # phpass hashes typically start with $P$ or $H$ and are 34 chars total
    ...
    # Extract salt from positions 4..12
    ...
    # Encode the final hash in the phpass format
    ...
    # phpass only stores 22 chars of the resulting base-64
    encoded = encoded[:22]
    output += encoded
...
def guess_hash_algorithm(hash_value: str) -> Optional[List[str]]:
    ...
    if hash_value.startswith("$argon2id$"):
        return ['argon2id']
    # Detect phpass MD5 based on prefix and fixed total length
...
def validate_hash_length(hash_algorithm: str, hash_value: str) -> bool:
    ...
    """
    if hash_algorithm in ['bcrypt', 'argon2id', 'scrypt', 'phpass_md5']:
        return True
...
def main() -> None:
    ...
    valid_algorithms = [
        ...
        'sha3_512', 'bcrypt', 'scrypt', 'argon2id', 'phpass_md5', 'auto'
    ]
    ...
    while True:
        ...
        if choice == '1':
            print("\n🔐  Password Cracker  🔐\n")
            print("Supported algorithms: md4, md5, phpass_md5, crc32, ripemd160, blake2_224, sha1, sha1_v2, sha224, sha256, sha512, sha3_224, sha3_256, sha3_384, sha3_512, bcrypt, scrypt, argon2id, or auto")
            ...
            # Quick format checks for special cases
            if hash_algorithm == 'bcrypt':
            ...
            if hash_algorithm not in ['bcrypt', 'argon2id', 'scrypt', 'phpass_md5']:
