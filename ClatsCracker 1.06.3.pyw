...
LOG_FILE = "Hash_Cracking_Results_ClatScope.txt" 
...
def check_password_cpu(password: str, hash_to_decrypt: str, hash_algorithm: str) -> Tuple[bool, Optional[str]]:
...
def process_dictionary_chunk(chunk: List[str], hash_to_decrypt: str, hash_algorithm: str, progress_interval: int) -> Tuple[int, Optional[str]]:
    processed = 0
    found_password = None
    local_counter = 0
    for pwd in chunk:
...
def process_brute_force_chunk(chunk: List[str], hash_to_decrypt: str, hash_algorithm: str, progress_interval: int) -> Tuple[int, Optional[str]]:
    processed = 0
    found_password = None
    local_counter = 0
    for pwd in chunk:
...
def generate_passwords(cset: str, plen: int, chunk_size: int = 100000):
    batch = []
...
class HashCrackerGUI:
    def __init__(self, master):
        self.master = master
        master.title("ClatsCracker Hash Tool GUI v1.06.3")
...
╚██████╗███████╗██║  ██║   ██║   ███████║╚██████╗██║  ██║██║  ██║╚██████╗██║  ██╗███████╗██║  ██║
 ╚═════╝╚══════╝╚═╝  ╚═╝   ╚═╝   ╚══════╝ ╚═════╝╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝
           C L A T S C R A C K E R     H A S H     T O O L   (Version 1.06.3)
